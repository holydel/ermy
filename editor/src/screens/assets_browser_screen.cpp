...
AssetsSelectionWithDeletion* gAssetsSelectionWithDeletion = nullptr;
...
void DrawAssetsFolders(AssetFolder* folder)
{
	...
	{
		if (gSelectedFolder != folder)
		{
			if (gAssetsSelectionWithDeletion != nullptr)
				delete gAssetsSelectionWithDeletion;

			gAssetsSelectionWithDeletion = new AssetsSelectionWithDeletion();

			PropertyEditorScreen::Instance()->SetProps(folder);
		}
...
void UpdateLayoutSizes(float avail_width)
{
	...
	LayoutLineCount = (gSelectedFolder->content.size() + LayoutColumnCount - 1) / LayoutColumnCount;
...
void DrawFolderContents(AssetFolder* folder)
{
	...
	ImGuiMultiSelectIO* ms_io = ImGui::BeginMultiSelect(ms_flags, Selection.Size, folder->content.size());
	...
	const int item_curr_idx_to_focus = want_delete ? Selection.ApplyDeletionPreLoop(ms_io, folder->content.size()) : -1;
	RequestDelete = false;
	...
	while (clipper.Step())
	{
		for (int line_idx = clipper.DisplayStart; line_idx < clipper.DisplayEnd; line_idx++)
		{
			...
			const int item_max_idx_for_current_line = IM_MIN((line_idx + 1) * column_count, folder->content.size());
